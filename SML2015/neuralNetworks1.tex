...
\script{Chapter 11 in ESL is a good introductory reference for neural networks}
\end{frame}

\transitionSlide{Background}
...
\begin{itemize}
\item taking atomic units known as \alg{neurons}, which can either be ``on'' or ``off''
\item putting them in \alg{networks} with each other, where the \alo{signal} is given by which neurons are ``on''
at a given time
\end{itemize}
...
\begin{frame}[fragile]
\frametitle{Background}
In the mid 2000's, new approaches for \alo{initializing} neural networks became available

\vsp
These approaches are collectively known as \alg{deep learning}
...
\begin{itemize}
\item A \alo{fixed basis}, $(\phi_k)_{k=1}^\infty$
...
\item Truncate this expansion\footnote{Often higher $k$ are more \alo{rough} $\Rightarrow$ this is a smoothness assumption} at $K$
\[
...
Write $L$ as the \alo{link function}
\vsp
...
%& =
% \beta_0 + \sum_{j=1}^J \beta_j \sigma\left(\alpha_{k0} + \alr{\sum_{j=1}^p\alpha_{kj}x_j}\right)
\end{align*}

\smallCapGreen{Compare:} A nonparametric GLM would have the form
\[
...
\begin{itemize}
\item The derived features $\alb{Z_k = \sigma(\alpha_{k0} + \alpha_k^{\top}X)}$ and are called the \alg{hidden units}
\begin{itemize}
\item The function $\alb{\sigma}$ is called the \alg{activation function} and is very often $\sigma(u) = (1 + e^{-u})^{-1}$

\script{This particular $\sigma(u)$ is known as the \alg{sigmoid function}}
...
\begin{frame}[fragile]
\frametitle{High level overview}
\smallCapGreen{Example:} If $L(\mu) = \mu$, then we are doing regression:
\[
\mu(X) = \beta_0 + \sum_{k=1}^K \beta_k \sigma\left(\alpha_{k0} + \sum_{j=1}^p\alpha_{kj}x_j\right)
\]

but in a \alo{transformed space}

\vsp
\smallCapGreen{Two observations:}
\begin{itemize}
\item The $\sigma$ function generates a \alo{feature map}
...
We start with $p$ \alo{covariates}

\vsp
We generate $K$ \alo{features} 

\vsp
\smallCapGreen{Example:} GLMs with a \alo{feature} transformation
\begin{align*}
\Phi(X) 
& = 
...
\begin{frame}[fragile]
\frametitle{Observation 2: Activation function}
Plot of \alo{sigmoid} activation function
\begin{figure}
\centering
\includegraphics[width=2.5in]{../figures/sigmoidPlot.pdf}
\end{figure}
...
\begin{itemize}
\item \smallCapGreen{Regression:} The link function is $L(u) = u$

\script{here, $G=1$}
\item \smallCapGreen{Classification:} With $G$ classes, we are modeling $\pi_g = \P(Y = g|X)$ and $L = \textrm{logit}$:
\[
...
Neural networks have \alo{many} (\alo{MANY}) unknown parameters
...
\smallCapGreen{Total parameters:} $\asymp Kp + GK = K(p + G)$
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Training neural networks}
We'll need some derivatives to implement the \alo{gradient descent}
...
%\parenthetical{\quad}{\textrm{This later evaluation only matters if $L(x) \neq x$}}\\
\hat{\alpha}_{kj}^{t+1} 
& = 
...
\item As the updates are only in terms of $\hat{R}_i$, the algorithm can be run in either \alo{batch} or \alo{online}
mode
\end{itemize}
...
\smallCapGreen{Note:}  In either case, we now solve:
\[
...
\item \smallCapGreen{Number of hidden units ($K$):} It is generally better to have too many hidden units
than too few (regularization can eliminate some).  

\script{Later, we will see this can include adding multiple hidden layers}
\end{itemize}
\end{frame}
