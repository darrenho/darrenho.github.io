...
PROC GLIMMIX DATA = soil PLOTS = (residualpanel pearsonpanel studentpanel)	OUTDESIGN(X) = nestedX;
	CLASS texture soil method;
 	MODEL Y = texture soil(texture) method texture*method method*soil(texture) / dist = beta link = logit E3;
RUN;
 
 TITLE 'Fixed effects model unnested';
PROC GLIMMIX DATA = soil PLOTS = (residualpanel pearsonpanel studentpanel) OUTDESIGN(X) = unnestedX;
	CLASS texture soil method;
 	MODEL Y = texture soil method texture*method method*soil / dist = beta link = logit E3;
RUN;