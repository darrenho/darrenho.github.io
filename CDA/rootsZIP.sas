...
/*Get the zip fit with a contrast for photo=8, bap go from 17.6 to 8.8*/
proc genmod data=Trajan;
   class bap(ref=last) photoperiod(ref=last) / PARAM=REF;
   model roots = bap|photoperiod / dist=zip offset=lshoot;
   zeromodel photoperiod;
   CONTRAST 'effect CI' intercept 0 bap 0 0 1 bap*photoperiod 0 0 1 / E;
   ESTIMATE 'effect CI' intercept 0 bap 0 0 1 bap*photoperiod 0 0 1;
